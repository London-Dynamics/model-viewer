...
import { Group, Object3D, Material as Material$4, Texture as Texture$4, Vector3 as Vector3$1, Mesh as Mesh$1, WebGLRenderer, Scene as Scene$1, Camera as Camera$2, AnimationActionLoopStyles, PerspectiveCamera as PerspectiveCamera$1, OrthographicCamera as OrthographicCamera$1, Box3, Sphere, ToneMapping, Vector2, Euler, AnimationMixerEventMap, XRTargetRaySpace, EventDispatcher, Event, Intersection, MeshPhysicalMaterial, Quaternion as Quaternion$1, EulerOrder } from 'three';
...
import { Part, SnapPoint } from '@london-dynamics/types/planner';
...
declare class Shadow extends Object3D {
    ...
    renderFloor(renderer: WebGLRenderer, scene: Scene$1, camera: Camera$2): void;
...
type ShadowMode = 'none' | 'soft-shadow' | 'path-tracer';
/**
...
declare class ModelScene extends Scene$1 {
    ...
    camera: PerspectiveCamera$1 | OrthographicCamera$1;
    xrCamera: Camera$2 | null;
    private cameraType;
    private savedPerspectiveFov;
    url: string | null;
    pivot: Object3D<three.Object3DEventMap>;
    target: Object3D<three.Object3DEventMap>;
    environmentRoot: Object3D<three.Object3DEventMap>;
    ...
    private setupScene;
    reset(): void;
    setEnvironmentModel(model: Object3D | null, dispose?: () => void): void;
    clearEnvironmentModel(): void;
    setEnvironmentModelVisible(visible: boolean): void;
    dispose(): void;
    ...
    setSkyboxRotation(rotation: Euler): void;
    ...
    setShadowMode(shadowMode: ShadowMode | string): void;
    /**
     ...
     */
    updateOrthographicFrustum(halfHeight?: number): void;
...
type CameraControlMode$1 = 'orbit' | 'fps';
declare interface LDControlsInterface extends ControlsInterface {
    cameraControlMode: CameraControlMode$1;
    fpsKeyboardMove: boolean;
    fpsFlyMode: boolean;
    fpsLookSensitivity: number;
    fpsMoveSensitivity: number;
    setCameraControlsMode(mode: CameraControlMode$1, options?: {
...
declare interface LDDebugInterface {
    debug: boolean;
}
type LogFunction = (...args: any[]) => void;
type WarnFunction = (...args: any[]) => void;
type ErrorFunction = (...args: any[]) => void;
...
declare interface LDAnimationInterface {
...
declare interface LDEnvironmentInterface {
...
type SkyboxRotationAxis = 'x' | 'y' | 'z';
declare interface LDSkyboxRotationInterface {
...
type CameraView = CameraMeta['object'] | {
...
declare interface LDCameraInterface {
    ...
    animateCameraTo(view: CameraView, options?: CameraAnimationOptions): Promise<void>;
...
type CaptureImageFit = 'cover' | 'contain' | 'fill';
/** Normalized focal point used when cropping (0–1 in each dimension). */
interface CaptureImageFocalPoint {
    x: number;
    y: number;
}
/**
...
declare interface LDImageCaptureInterface {
    /**
...
declare interface LDLightsInterface {
    lights: boolean;
...
type RectangleSelectionMode = 'replace' | 'add' | 'remove' | 'toggle';
interface LDSelectionInterface {
    disableBaseModelSelection: boolean;
    selectAll(): void;
    deselectAll(): void;
    applyRectangleSelection(rect: DomRectLike, options?: {
        mode?: RectangleSelectionMode;
    }): void;
}

type SurfaceType = 'floor' | 'wall' | 'ceiling';

type ClipboardEntryKind = 'part' | 'group' | 'selection';
type SelectionClipboardItem = {
    itemEntry: ClipboardEntry;
    anchorOffset: Vector3$1;
};
type SelectionClipboardPayload = {
    items: SelectionClipboardItem[];
    leaderIndex: number;
};
type GroupChildClipboardPayload = {
    oldName: string;
    part?: Partial<Part>;
    selection?: Selection;
    snapPoints?: SnapPoint[];
    selectable?: boolean;
    localPosition: Vector3$1;
    localQuaternion: Quaternion$1;
    localScale: Vector3$1;
};
type GroupClipboardPayload = {
...
type ClipboardPasteSessionState = {
    active: boolean;
...
type PlacementPoseHost = {
    ...
    applySurfaceSnapForPlacement?(object: Object3D, clientX: number, clientY: number): SurfaceSnapHit | null;
    _getCursorWorldPosition?(): {
        x: number;
        y: number;
        z: number;
    } | null;
...
type PasteSessionState = 'previewing' | 'committing' | 'ended' | 'cancelled';
type PasteCommitResult = {
    id: string;
    node: Object3D;
    nodes?: Object3D[];
};
type PasteSessionHost = PlacementPoseHost & {
    [$needsRender](): void;
    applySurfaceSnapForPlacement(object: Object3D, clientX: number, clientY: number): SurfaceSnapHit | null;
    _finalizePasteCommit(node: Object3D, entry: ClipboardEntry, options?: {
        select?: boolean;
        emitChange?: boolean;
    }): PasteCommitResult;
    _finalizePasteCommitMany?(commits: Array<{
        node: Object3D;
        itemEntry: ClipboardEntry;
    }>, options?: {
        select?: boolean;
 ...
 */
declare class PasteSession extends EventTarget {
    readonly id: string;
    ...
    private readonly _placementItems;
    private _leaderIndex;
    private _hasValidSurfaceSnap;
    ...
    commit(options?: {
        select?: boolean;
        emitChange?: boolean;
    }): PasteCommitResult | null;
    cancel(): void;
    dispose(): void;
    private _copySurfaceMetadata;
    private _removeGhosts;
}

type TransformSource = 'pointer-drag' | 'rotation-disc-y' | 'api' | 'animation' | 'align-distribute';

type HistoryEntryKind = 'transform' | 'add' | 'remove' | 'structure';
...
type AlignAction = 'align-left' | 'align-center-h' | 'align-right' | 'align-top' | 'align-center-v' | 'align-bottom' | 'distribute-h' | 'distribute-v' | 'distribute-line' | 'equal-gap-h' | 'equal-gap-v' | 'equal-gap-line';
...
type BulkReplaceItem = {
    objectUuid: string;
    src?: string;
} & Omit<PlacementOptions, 'getLowResUrl' | 'getHighResUrl'>;
type PlacementSessionLike = EventTarget & {
    id: string;
...
type LoadFunction = (src: string, id: string) => Promise<GLTF$2>;
type LoadManyFunction = (items: Array<{
    src: string;
    id: string;
}>) => Promise<GLTF$2[]>;
type AttachFunction = (objectId: string, targetId?: string, options?: PositionOptions) => void;
type AttachMaterialFunction = (materialId: string, targetId: string) => void;
type ClearSceneFunction = () => void;
type PointerInteractionKind = 'none' | 'rotation-control-y' | 'selected-object-drag' | 'interactive-session';
type PointerInteractionTarget = {
...
type PasteResult = PasteCommitResult;
...
declare interface LDModularInterface {
    load: LoadFunction;
    ...
    setRotation(objectName: string, anglesDegrees: [number, number, number], options?: RotationOptions): void;
    setRotation(anglesDegrees: [number, number, number], options?: RotationOptions): void;
    setScale(objectName: string, value: [number, number, number]): void;
    setScale(value: [number, number, number]): void;
    setRotationX(objectName: string, x: number | string, options?: RotationOptions): void;
    setRotationX(x: number | string, options?: RotationOptions): void;
    setRotationY(objectName: string, y: number | string, options?: RotationOptions): void;
    setRotationY(y: number | string, options?: RotationOptions): void;
    setRotationZ(objectName: string, z: number | string, options?: RotationOptions): void;
    ...
    beginPlacement: (lowResSrc: string | undefined, highResSrc: string | undefined, options?: PlacementOptions, initialMouse?: {
        clientX: number;
        clientY: number;
    }) => PlacementSession;
    placeGlb: (highResSrc?: string, options?: PlacementOptions & ImmediatePlacementTransform) => Promise<{
        id: string;
        node: Object3D;
    }>;
    placeManyGlb: (items: BulkPlacementItem[], options?: {
        concurrency?: number;
    ...
    replacePart: (objectUuid: string, src?: string, options?: PlacementOptions) => Promise<void>;
    replaceManyParts: (items: BulkReplaceItem[], options?: {
        concurrency?: number;
    ...
    getPointerInteractionTarget(clientX: number, clientY: number): PointerInteractionTarget;
    ...
    clearSelection?: () => void;
    removeObject: (objectName: string, options?: {
        animate?: boolean;
    }) => void;
    removeSelectedObjects: (options?: {
        animate?: boolean;
    }) => void;
    ...
    copyPart: (objectUuid?: string, options?: CopyPartOptions) => PasteSession | void;
 ...
 */
declare class PlacementSession extends EventTarget {
    id: string;
    state: 'placing' | 'loading' | 'ended' | 'cancelled';
    placeholder: Object3D | null;
    private log;
    ...
    private _targetBottomCenter;
    private _hasValidSurfaceSnap;
    private _placeholderLoadEmitted;
    constructor(element: any, log: LogFunction, warn?: WarnFunction, error?: ErrorFunction, lowResSrc?: string, highResSrc?: string, options?: PlacementOptions, _immediatePlacement?: boolean);
    private _emitPlaceholderLoaded;
    _ensurePlaceholderReady(): Promise<void>;
    private _getWallCursorPointFromPlaceholder;
    requiresSurfaceSnap(): boolean;
    hasValidSurfaceSnap(): boolean;
    private _createPlaceholderFromBounds;
    private _createPlaceholderAnchor;
    _loadPlaceholder(): Promise<void>;
    updatePosition(clientX: number, clientY: number): void;
    /**
     ...
     */
    applyFallbackCommitPosition(): void;
    commit(finalSrc?: string): Promise<{
        id: string;
        node: any;
    } | undefined>;
    private _placeFinalGlb;
    cancel(): void;
    private _cleanupPlaceholder;
    private _endInteractive;
}

type AONoiseType = 'magic-square' | 'random';
...
type AoAlgorithmName = 'ssao' | 'sao' | 'n8ao' | 'hbao' | 'gtao';
type AoOutputName = 'default' | 'diffuse' | 'depth' | 'normal' | 'ao' | 'denoise';
declare interface LDAmbientOcclusionInterface {
...
declare class LDPathTracerComposer implements EffectComposerInterface {
    private renderer;
    private scene;
    private camera;
    private readonly physicalCamera;
    private readonly denoiseMaterial;
    private readonly denoiseQuad;
    private pathTracer;
    private ambientOcclusionPreviewComposer;
    private currentAmbientOcclusionPreviewOptions;
    private queuedRender;
    private readonly lastCameraWorldMatrix;
    private readonly lastCameraProjectionMatrix;
    private readonly meshWorldMatrices;
    private readonly shadowCatcherCenter;
    private shadowCatcher;
    private shadowCatcherState;
    private cameraStateInitialized;
    private options;
    private needsSceneUpdate;
    ...
    render(_deltaTime?: DOMHighResTimeStamp): void;
    updateOptions(partial: Partial<PathTracerOptions>): void;
    getSamples(): number;
    get ambientOcclusionPreviewOptions(): AmbientOcclusionOptions | null;
    markSceneDirty(): void;
    dispose(): void;
    ...
    private rememberCameraState;
    private getPathTracerCamera;
    private queueNextProgressiveRender;
    private requestRender;
}

declare interface LDPathTracerInterface {
    pathTracer: boolean;
...
type PathTracerDenoiseMode = 'off' | 'gpu' | 'server-ai';
interface ServerAIDenoiseOptions {
    endpoint?: string;
    image?: string;
    model?: string;
...
interface PathTracerServerAIDenoiseOptions extends ServerAIDenoiseOptions {
    samples?: number;
}
declare interface LDServerAIDenoiseInterface {
    ...
    pathTracerServerAIDenoise(options?: PathTracerServerAIDenoiseOptions): Promise<string>;
}

type LDBloomMode = 'unreal' | 'classic';
type LDBloomQualityMode = 'performance' | 'quality' | 'smart';
type LDBloomTargetKind = 'material' | 'mesh';
interface LDBloomTarget {
    material?: string;
    mesh?: string;
    color?: string;
    intensity?: number;
...
declare interface LDBloomInterface {
    bloom: boolean;
    ...
    setBloomTargetEnabled(kind: LDBloomTargetKind, name: string, enabled: boolean): void;
    getSceneNames(): {
        objects: string[];
        materials: string[];
    };
...
declare interface LDRenderPipelineInterface {
    readonly ldRenderPipelineActive: boolean;
}

declare interface LDMeasureInterface {
    measure: boolean;
    gridShapes: boolean;
    addGridShapeLines(paths: Array<Array<[number, number]>> | Array<[number, number]>, options?: {
        id?: string;
        color?: string | number;
        thickness?: number;
...
interface FloatingObjectAnchorInterface {
    disableFloatingControls: boolean;
    selectObjectForControls(object: Object3D): void;
    clearSelectedObject(): void;
}

declare interface LDMaterialManagerInterface {
    setMaterial(materialName: string, properties: any, resetBeforeApply?: boolean): void;
}

declare const ModelViewerElementImpl: {
    new (...args: any[]): LDMaterialManagerInterface;
    prototype: LDMaterialManagerInterface;
} & object & {
    new (...args: any[]): FloatingObjectAnchorInterface;
    prototype: FloatingObjectAnchorInterface;
} & {
    new (...args: any[]): LDMeasureInterface;
    prototype: LDMeasureInterface;
} & {
    new (...args: any[]): LDRenderPipelineInterface;
    prototype: LDRenderPipelineInterface;
} & {
    new (...args: any[]): LDBloomInterface;
    prototype: LDBloomInterface;
} & {
    new (...args: any[]): LDServerAIDenoiseInterface;
    prototype: LDServerAIDenoiseInterface;
} & {
    new (...args: any[]): LDPathTracerInterface;
    prototype: LDPathTracerInterface;
} & {
    new (...args: any[]): LDAmbientOcclusionInterface;
    prototype: LDAmbientOcclusionInterface;
} & {
    new (...args: any[]): LDModularInterface;
    prototype: LDModularInterface;
} & {
    new (...args: any[]): LDSelectionInterface;
    prototype: LDSelectionInterface;
} & {
    new (...args: any[]): LDLightsInterface;
    prototype: LDLightsInterface;
} & {
    new (...args: any[]): LDImageCaptureInterface;
    prototype: LDImageCaptureInterface;
} & {
    new (...args: any[]): LDCameraInterface;
    prototype: LDCameraInterface;
} & {
    new (...args: any[]): LDSkyboxRotationInterface;
    prototype: LDSkyboxRotationInterface;
} & {
    new (...args: any[]): LDEnvironmentInterface;
    prototype: LDEnvironmentInterface;
} & {
    new (...args: any[]): LDAnimationInterface;
    prototype: LDAnimationInterface;
} & {
    new (...args: any[]): LDDebugInterface;
    prototype: LDDebugInterface;
} & {
    ...
    new (...args: any[]): LDControlsInterface;
    prototype: LDControlsInterface;
} & {
...
type ModelViewerElement = InstanceType<typeof ModelViewerElementImpl>;
