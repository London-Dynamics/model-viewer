...
function reconcileCameraControlsPose(cc: any, data: any, scene: any): void {
  ...
  const controlsZoom = (snapshot?.zoom ?? pose?.zoom ?? data.controlsZoom) as
    | number
    | undefined;
...
function restoreCameraFromControlsJSON(
  ...
  const hasSnapshotObject =
    data.controlsSnapshot != null && typeof data.controlsSnapshot === 'object';
...
function cameraPoseFromView(scene: any, view: CameraView): CameraPose | null {
  ...
  return {
    ...
    target,
    fov:
      typeof data.fov === 'number' && Number.isFinite(data.fov)
        ? data.fov
        : undefined,
  };
}
